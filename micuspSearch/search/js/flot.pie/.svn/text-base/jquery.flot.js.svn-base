...
(function($) {
    function Plot(target, data_, options_, plugins) {
        ...
        var series = [],
		  ...
		  colors: ["#CCCCCC", "#CCCCCC", "#CCCCCC", "#CCCCCC", "#CCCCCC", "#CCCCCC", "#CCCCCC"],
                legend: {
                    show: true,
	 ...
	 plot.makeCanvas = makeCanvas;
	 plot.canvasHeight = canvasHeight;
	 plot.canvasWidth = canvasWidth;
	 ...
	 plot.plotOffsetLeft = plotOffset.left;
	 plot.plotOffsetTop = plotOffset.top;
	 plot.selectionIsSane = selectionIsSane;
	...
	function insertLabels() {
		...
		addLabels(axes.xaxis, function (tick, axis) {
			return '<div id="' + tick.label + '" style="position:absolute;top:' + (plotOffset.top + plotHeight + margin) + 'px;left:' + Math.round(plotOffset.left + axis.p2c(tick.v) - axis.labelWidth/2) + 'px;width:' + 20 + 'px;margin-left:' + 5.75 + '%;margin-right:' + 5.75 +'%;text-align:center" class="unHighlightedHist tickLabelX">' + tick.label + "</div>";
		});
	...
	function drawOverlay() {
        ...
        function drawBarHighlight(series, point) {
            ...
            octx.strokeStyle = parseColor(series.highlightBorderColor).scale(1, 1, 1, series.highlightBorderOpacity).toString();
            var fillStyle = parseColor(series.highlightColor).scale(1, 1, 1, series.highlightOpacity).toString();
            var barLeft = series.bars.align == "left" ? 0 : -series.bars.barWidth/2;
